...
__global__ void normalizeMassPos(vector<Mass> Masses, float* massPoints, float fieldDepth){
...
__global__ void normalizeSprings(vector<Spring> Springs, float* springPoints, float fieldDepth){
	int i = blockIdx.x * blockDim.x + threadIdx.x;
	springPoints[i] = Springs[ i / 6].m1->Position.x / fieldDepth;
}

//calcuates force for each spring
__global__ void calcSpringForce(vector<Spring> Springs){
	int i = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void gravityForce(vector<Mass> m, float gravConst, float dt){
	int i = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void initMassAccel(vector<Mass> m){
	int i = blockIdx.x * blockDim.x + threadIdx.x;
	m[i].accel.x = 0.0f;
	m[i].accel.y = 0.0f;
	m[i].accel.z = 0.0f; 
